...
## 6. 人間確認UI — 最後の砦（実装済 `OcrSaveSection` を基盤に）

自動化が進んでも、**危険なものだけ確実に人が見る**導線が事故を防ぐ。

- **1注文単位**で承認（マトリックスは注文ごとにカード分割）。手動OCRは実装済み。
- **赤=危険を一目で**：confidence<0.7 行・名寄せ失敗・数量2.5倍超を赤背景＋⚠️。
- **取引先/商品はAI読取り名でプリセット**しつつ手修正可（OcrSaveSection 実装済）。
- **納品日必須**（不明確定は許さない）。
- 承認＝`orders` INSERT（status は admin直接確認扱いで `approved`）。修正は learning へ。
- pending_review 一覧（admin/inbox）で**未処理の山を可視化**し、取りこぼしを防ぐ。
